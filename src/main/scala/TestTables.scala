...
object TestTables {
  ...
  def programmingLanguages1: Table = {
    val newF = NewCol("Functional", "Yes", Value(tableFunctional))
    val newI = NewCol("Imperative", "Yes", Value(tableImperative))
    val newOO = NewCol("Object-Oriented", "Yes", Value(tableObjectOriented))
    val aux = Merge("Language", newF, newOO)
    val final_query = Merge("Language", aux, newI)
    final_query.eval.get
  }

  // 3.2
  val programmingLanguages2: Table = {
    Filter(And(Field("Original purpose", x => x.contains("Application")), Field("Other paradigms", x => x.contains("concurrent"))),Value(programmingLanguages1)).eval.get
